...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput):
        ...
        """方法1"""
        output = YOLOoutput.transpose(1, 2).contiguous()
        ...
        score_mask = max_conf.data > 0.2
        max_conf = max_conf * score_mask
        ...
        # confs_if_object = confs_for_class * output_objectness
        # confs_if_object = self.config.loss_target(output_objectness, confs_for_class)
        # score_mask = confs_if_object.data >= 0.25
        # score = confs_if_object * score_mask
        # if (score_mask.sum(dim = 1)!=0).all():
        #     a = score.sum(dim=1)
        #     b = score_mask.sum(dim = 1)
        #     # max_conf = (score.sum(dim=1)/score_mask.sum(dim = 1))
        #     max_conf = score.sum(dim=1)
        # else:
        #     max_conf = torch.zeros_like(score.sum(dim=1))

        """方法3"""
        # output_objectness = YOLOoutput[:, :, 4]  # shape:torch.Size([8, 14175])
        # # output_objectness=torch.sigmoid(YOLOoutput[:,:, 4]) #shape:torch.Size([8, 14175])
        # confs_for_class = YOLOoutput[:, :, 5]  # 这种适合只有一个类别的情况，若有多个类别，可参考下面注释掉的写法
        # # confs_if_object=confs_for_class * output_objectness
        # confs_if_object = self.config.loss_target(output_objectness, confs_for_class)
        # score_mask = confs_if_object.data > 0.1  # shape:torch.Size([8, 14175])
        # score = confs_if_object * score_mask  # shape:torch.Size([8, 14175])
        # # max_conf, max_conf_idx = torch.max(confs_if_object, dim=1)
        
        # if (score_mask.sum(dim=1) != 0).all():  # score_mask.sum(dim=1)必须满足所有的元素都不为0
        #     max_conf = (score.sum(dim=1) / score_mask.sum(dim=1))
        #     # max_conf = score.sum(dim=1)
        # else:
        #     max_conf = torch.zeros_like(score.sum(dim=1))  #
...
class NPSCalculator(nn.Module):
    ...
    def forward(self, adv_patch, adv_mask, adv_p):
        # calculate euclidian distance between colors in patch and colors in printability_array 
        # square root of sum of squared difference
        color_dist = ((torch.mul(adv_patch, adv_mask) + adv_p) - (torch.mul(self.printability_array, adv_mask) + adv_p) + 0.000001)
...
class TotalVariation(nn.Module):
    ...
    def forward(self, adv_patch, adv_mask, adv_p):
        ...
        tvcomp1 = torch.sum(torch.abs((torch.mul(adv_patch, adv_mask)+adv_p)[:, :, 1:] - (torch.mul(adv_patch, adv_mask)+adv_p)[:, :, :-1]+0.000001),0)
        tvcomp1 = torch.sum(torch.sum(tvcomp1,0),0)
        tvcomp2 = torch.sum(torch.abs((torch.mul(adv_patch, adv_mask)+adv_p)[:, 1:, :] - (torch.mul(adv_patch, adv_mask)+adv_p)[:, :-1, :]+0.000001),0)
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        # mypad = nn.ConstantPad2d((int(13 + 0.5), int(13), int(13+ 0.5), int(13)), 1)
        # adv_mask = torch.tensor([0],dtype=torch.float).expand(210).unsqueeze(0).expand(90,-1).unsqueeze(0).expand(3,-1,-1).unsqueeze(0).expand(adv_patch.size(0), -1, -1 ,-1).cuda()
        # # adv_mask = torch.zeros_like(adv_patch).cuda()
        # adv_mask = mypad(adv_mask)
        # mypad = nn.ConstantPad2d((int(0 + 0.5), int(0), int(60 + 0.5), int(60)), 0)
        # adv_mask = mypad(adv_mask)

        mypad = nn.ConstantPad2d((int(20 + 0.5), int(20), int(20+ 0.5), int(20)), 1)
        adv_mask = torch.tensor([0],dtype=torch.float).expand(210).unsqueeze(0).expand(90,-1).unsqueeze(0).expand(3,-1,-1).unsqueeze(0).expand(adv_patch.size(0), -1, -1 ,-1).cuda()
        # adv_mask = torch.zeros_like(adv_patch).cuda()
        adv_mask = mypad(adv_mask)
        mypad = nn.ConstantPad2d((int(0 + 0.5), int(0), int(60 + 0.5), int(60)), 0)
        ...
        adv_mask1 = torch.tensor([0],dtype=torch.float).expand(158).unsqueeze(0).expand(250,-1).unsqueeze(0).expand(3,-1,-1).unsqueeze(0).expand(adv_patch.size(0), -1, -1 ,-1).cuda()
        mypad = nn.ConstantPad2d((int(46 + 0.5), int(46), int(0 + 0.5), int(0)), 1)
        adv_mask1 = mypad(adv_mask1)

        adv_mask2 = torch.tensor([0],dtype=torch.float).expand(250).unsqueeze(0).expand(38,-1).unsqueeze(0).expand(3,-1,-1).unsqueeze(0).expand(adv_patch.size(0), -1, -1 ,-1).cuda()
        mypad = nn.ConstantPad2d((int(0 + 0.5), int(0), int(106 + 0.5), int(106)), 1)
        adv_mask2 = mypad(adv_mask2)
        
        adv_mask = torch.mul(adv_mask, adv_mask1)
        adv_mask = torch.mul(adv_mask, adv_mask2)
        ...
        adv_mask = adv_mask.unsqueeze(0)#.unsqueeze(0)
        adv_batch = adv_patch.expand(lab_batch.size(0), lab_batch.size(1), -1, -1, -1) #每一个目标上对应一个patch
        adv_batch1 = adv_mask.expand(lab_batch.size(0), lab_batch.size(1), -1, -1, -1)
        batch_size = torch.Size((lab_batch.size(0), lab_batch.size(1))) #每个batch两张图片，每张图片上40个目标
        ...
        # cls = cls_mask[0,1,:,:,:]
        # cls = transforms.ToPILImage()(cls.detach().cpu())
        # cls.show()
        msk_batch = torch.cuda.FloatTensor(cls_mask.size()).fill_(1) - cls_mask
        # msk = msk_batch[0, 1, :, :, :]
        # msk = transforms.ToPILImage()(msk.detach().cpu())
        # msk.show()

        # Pad patch and mask to image dimensions
        mypad = nn.ConstantPad2d((int(padheight + 0.5), int(padheight), int(padwidth + 0.5), int(padwidth)), 0)
        adv_batch = mypad(adv_batch)
        adv_batch1 = mypad(adv_batch1)
        ...
        # Resizes and rotates
        current_patch_size_height = adv_patch.size(-1)
        current_patch_size_width = adv_patch.size(-2)
        lab_batch_scaled = torch.cuda.FloatTensor(lab_batch.size()).fill_(0)
        lab_batch_scaled[:, :, 1] = lab_batch[:, :, 1] * img_size  #计算图像中目标的真实位置。
        lab_batch_scaled[:, :, 2] = lab_batch[:, :, 2] * img_size
        lab_batch_scaled[:, :, 3] = lab_batch[:, :, 3] * img_size
        lab_batch_scaled[:, :, 4] = lab_batch[:, :, 4] * img_size
        # rand = random.uniform(0.2, 0.3)
        # target_size = torch.sqrt(((lab_batch_scaled[:, :, 3].mul(0.2)) ** 2) + ((lab_batch_scaled[:, :, 4].mul(0.2)) ** 2)) #计算图像中添加干扰块的大小。
        target_size = lab_batch_scaled[:,:,3:]
        target_x = lab_batch[:, :, 1].view(np.prod(batch_size))
        target_y = lab_batch[:, :, 2].view(np.prod(batch_size))
        targetoff_x = lab_batch[:, :, 3].view(np.prod(batch_size))
        targetoff_y = lab_batch[:, :, 4].view(np.prod(batch_size))
        if(rand_loc):
            off_x = targetoff_x*(torch.cuda.FloatTensor(targetoff_x.size()).uniform_(-0.1,0.1))
            target_x = target_x + off_x
            off_y = targetoff_y*(torch.cuda.FloatTensor(targetoff_y.size()).uniform_(-0.1,0.1))
            target_y = target_y + off_y
        # target_y = target_y - 0.05
        current_patch_size = torch.tensor([202, 88]).unsqueeze(0).expand(1, -1).unsqueeze(0).expand(lab_batch.size(0),-1,-1).cuda()
        scale = torch.div(target_size, current_patch_size)
        scale = torch.min(scale[:,:,0], scale[:, :, 1])
        scale = scale.view(anglesize)

        s = adv_batch.size()
        adv_batch = adv_batch.view(s[0] * s[1], s[2], s[3], s[4])
        adv_batch1 = adv_batch1.view(s[0] * s[1], s[2], s[3], s[4])
        ...
        adv_batch_t1 = F.grid_sample(adv_batch1, grid)
        ...
        adv_batch_t1 = adv_batch_t1.view(s[0], s[1], s[2], s[3], s[4])
        msk_batch_t = msk_batch_t.view(s[0], s[1], s[2], s[3], s[4])

        adv_batch_t = torch.clamp(adv_batch_t, 0.000001, 0.999999)
        # adv_batch_t1 = torch.clamp(adv_batch_t1, 0.000001, 0.999999)
        #img = msk_batch_t[0, 0, :, :, :].detach().cpu()
        #img = transforms.ToPILImage()(img)
        #img.show()
        #exit()

        return adv_batch_t * msk_batch_t * adv_batch_t1
...
class InriaDataset(Dataset):
    ...
    def __init__(self, img_dir, lab_dir, max_lab, imgsize, shuffle=True):
        ...
        self.img_paths = [] #存放图片的路径信息
        for img_name in self.img_names:
            self.img_paths.append(os.path.join(self.img_dir, img_name))
        self.lab_paths = [] #存放图片标注的路径信息
        for img_name in self.img_names:
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(self.img_dir, self.img_names[idx]).replace('\\','/')
        lab_path = os.path.join(self.lab_dir, self.img_names[idx]).replace('.jpg', '.txt').replace('.png', '.txt').replace('\\','/')
        image = Image.open(img_path).convert('RGB')
        if os.path.getsize(lab_path):       #check to see if label file contains data. 
            label = np.loadtxt(lab_path)
        else:
            label = np.ones([5])
        im_pt = img_path.split('/')[-1]
    ...
    def pad_and_scale(self, img, lab):
        ...
        if w==h:
            ...
            if dim_to_pad == 1:
                padding = (h - w) / 2 #图片的上下各补足一块
                padded_img = Image.new('RGB', (h,h), color=(127,127,127))
                padded_img.paste(img, (int(padding), 0))
                lab[:, [1]] = (lab[:, [1]] * w + padding) / h
                lab[:, [3]] = (lab[:, [3]] * w / h)
            else:
                padding = (w - h) / 2 #图片的左右各补一块
